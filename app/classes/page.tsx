    ...
    <div className="bg-gradient-to-b from-[#f5f7f5] via-white to-[#eef3ee]">
      <div className="mx-auto max-w-6xl px-4 py-12 space-y-8">
        <div className="space-y-3">
          <p className="text-sm font-semibold uppercase tracking-wide text-[var(--primary)]">classes</p>
          <h1 className="text-3xl font-bold text-slate-900 md:text-4xl">운영 중인 수업</h1>
          <p className="text-base text-slate-600">
            관심 있는 수업을 확인하고, 로그인 후 원하는 시간대를 신청하세요.
          </p>
        </div>

        {error ? (
          <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">
            수업 정보를 불러오는 중 문제가 발생했습니다. 잠시 후 다시 확인해 주세요.
          </div>
        ) : courseList.length === 0 ? (
          <div className="rounded-2xl border border-[var(--primary-border)] bg-white p-6 text-sm text-slate-700">
            아직 등록된 수업이 없습니다. 관리자가 수업을 등록하면 이곳에 표시됩니다.
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {courseList.map((course: CourseCard) => (
              <article
                key={course.id}
                className="overflow-hidden rounded-2xl border border-[var(--primary-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
              >
                <div className="relative h-44 w-full bg-slate-100">
                  <Image
                    src={course.image_url || fallbackCourseImage}
                ...
                <div className="space-y-2 p-4">
                  ...
                  <p className="line-clamp-2 text-sm text-slate-600">
                    {course.description || "간단한 소개가 준비 중입니다."}
                  </p>
                  <Link
                    href="/auth/login"
