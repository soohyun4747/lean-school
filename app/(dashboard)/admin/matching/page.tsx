...
export default async function AdminMatchingPage() {
  ...
  const { data: matches } = await supabase
    .from("matches")
    .select("id, course_id, slot_start_at, slot_end_at, instructor_id, status, match_students(student_id)")
  ...
  const matchRows: MatchRow[] = (matches as MatchRow[] | null) ?? [];
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          {(matches ?? []).length === 0 && <p className="text-slate-600">매칭 내역이 없습니다.</p>}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">
                      {courseMap.get(match.course_id) ?? "수업"} · {formatDateTime(new Date(match.slot_start_at))}
                    </p>
                ...
                <div className="mt-2 text-xs text-slate-700">
                  ...
                  <ul className="mt-1 list-disc space-y-1 pl-5">
                    {match.match_students?.map((ms) => (
                      <li key={ms.student_id}>{profileMap.get(ms.student_id)?.name ?? ms.student_id}</li>
                    ))}
                  </ul>
                </div>
              </div>
        ...
        <CardContent>
          <form action={sendEmailBatch} className="space-y-3">
            ...
            <div>
              ...
              <Textarea name="body" rows={4} placeholder="매칭 결과 안내 템플릿을 입력하세요." required />
            </div>
