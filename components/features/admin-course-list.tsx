...
export function AdminCourseList({ courses }: CourseListProps) {
        const initialOrderRef = useRef<string[]>(courses.map((course) => course.id));
        ...
        const updateOrderState = (updater: (prev: ICourse[]) => ICourse[]) => {
                setCurrentCourses((prev) => {
                        ...
                        setHasChanges(initialOrderRef.current.join('|') !== next.map((course) => course.id).join('|'));
                        setStatus(null);
                        return next;
                });
        };
        ...
        const handleSave = () => {
                startTransition(async () => {
                        const result = await reorderCourses(currentCourses.map((course) => course.id));
                        if (!result?.success) {
                                setStatus({
                                        type: 'error',
                                        message: result?.error ?? '수업 순서를 저장하지 못했습니다.',
                                });
                                return;
                        }
                        ...
                        setHasChanges(false);
                        setStatus({ type: 'success', message: '수업 순서가 저장되었습니다.' });
                });
        };

        if (currentCourses.length === 0) {
                return <p className='text-sm text-slate-600'>등록된 수업이 없습니다.</p>;
        }
                                ...
                                {currentCourses.map((course, index) => (
                                        <div
                                                ...
                                                className={cn(
                                                        ...
                                                        draggingId === course.id && 'ring-2 ring-[var(--primary)]'
                                                )}>
                                                <div className='flex items-center gap-2'>
                                                        ...
                                                        <span className='text-xs text-slate-500'>현재 {index + 1} / {courseCount}</span>
                                                ...
                                                <Link href={`/admin/courses/${course.id}`} className='flex gap-3 group'>
