...
export function CourseEditForm({ course, instructors, windows }: Props) {
  ...
  const initialState: CourseUpdateResult = { success: false, error: undefined };
  ...
  return (
    <div className="space-y-4">
      <form action={formAction} className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <Input name="grade_range" defaultValue={course.grade_range} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <Select name="duration_minutes" defaultValue={course.duration_minutes.toString()}>
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-2">
            ...
            {course.image_url && (
              <p className="text-xs text-slate-600">현재 이미지가 유지됩니다. 새 이미지를 업로드하면 교체됩니다.</p>
            )}
            {!course.image_url && (
        ...
        <div className="md:col-span-2 flex items-center justify-end gap-2">
          <Button type="button" variant="secondary" onClick={() => router.push(`/admin/courses/${course.id}`)}>
            취소
          </Button>
      ...
      <div className="rounded-md border border-amber-100 bg-amber-50 p-3 text-xs text-amber-700">
        시간 정보를 수정하면 기존 슬롯과 신청 시간 선택이 초기화될 수 있습니다. 수정 전 확인해주세요.
      </div>
    </div>
  );
}
